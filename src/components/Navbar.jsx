...
const HIRE_ME_MESSAGE = "Hello Tamizharasi, I came across your portfolio and would like to connect with you regarding a creative design / campaign opportunity.";
...
const Navbar = () => {
  ...
  return (
    <>
      {/* Clipboard Toast */}
      <div
        className={`fixed top-6 left-1/2 -translate-x-1/2 z-[99999] px-6 py-3 rounded-full bg-[#FA2A55] text-white text-sm font-bold shadow-[0_8px_30px_rgba(250,42,85,0.4)] transition-all duration-500 pointer-events-none whitespace-nowrap ${
          ...
          >
            ...
            <div className="flex gap-3 flex-wrap">
              <a
                ...
                onClick={() => setFallbackVisible(false)}
                className="flex-1 text-center px-5 py-3 rounded-full bg-[#FA2A55] text-white font-bold hover:bg-rose-600 transition-all"
              >
                Open LinkedIn
              </a>
              <button
      ...
      >
        <div className="max-w-7xl mx-auto px-6 md:px-12 flex justify-between items-center">
          ...
          <div className="flex items-center">
            <a href="#" className="text-white text-2xl font-black tracking-tight whitespace-nowrap">
              {personalInfo.brandName}<span className="text-[#FA2A55]">.</span>
            </a>
          </div>
          ...
          <div className="hidden md:flex space-x-8">
            {navLinks.map((link) => (
              ...
              >
                {link.name}
                <span className="absolute -bottom-1 left-0 w-0 h-0.5 bg-[#FA2A55] transition-all duration-300 group-hover:w-full"></span>
              </a>
            ))}
          </div>
          ...
          <div className="hidden md:block">
            <button
              ...
              className="px-6 py-2.5 rounded-full bg-white/10 border border-white/20 text-white font-semibold hover:bg-white/20 hover:shadow-[0_0_15px_rgba(250,42,85,0.2)] transition-all duration-300 backdrop-blur-md cursor-pointer"
            >
              Collaborate
            </button>
          </div>
        ...
        <div
          className={`md:hidden absolute top-full left-0 w-full transition-all duration-300 overflow-hidden ${
            isOpen ? 'max-h-96 py-4 opacity-100 bg-[#FA2A55] shadow-2xl' : 'max-h-0 opacity-0 bg-transparent'
          }`}
        >
          <div className="flex flex-col px-6 space-y-4">
            ...
            <div className="pt-4 pb-2">
              <button
                ...
                className="inline-block px-6 py-3 rounded-full bg-white text-[#FA2A55] font-black hover:bg-black hover:text-white transition-colors w-full text-center shadow-lg cursor-pointer"
              >
                Collaborate
              </button>
            </div>
